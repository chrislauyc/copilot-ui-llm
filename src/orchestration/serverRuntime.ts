...
export { db } from './db';
export { appendEscalation, getPendingEscalation, getEscalations } from './escalationStore';
...
// Global middleware to log all HTTP responses and errors
  ...
  // OpenRouter/Anthropic/OpenAI/Gemini provider proxy route -- lives in
  // agentCore since it's needed to run a single agent session standalone.
  // Re-exports `setActiveOpenRouterSessionId` below for the external
  // scripts (review-pr.ts, audit-codebase.ts, etc.) that import it from
  // here.
  mountProviderProxyRoute(app, writeLog);
