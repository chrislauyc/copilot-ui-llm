...
import { buildAuditorSessionSettingsPure, ToolDefinition } from './auditorHelper.pure';
 ...
 */
/**
 * Thin, impure wrapper around buildAuditorSessionSettingsPure (issue #320):
 * computes the declarative settings shape (model/provider/systemMessage/
 * tool metadata) via the pure function, then attaches the two things that
 * make this session actually able to do anything -- the `onResult`-closing
 * submission-tool handler and `makeAuditorExecToolHandler`'s
 * `getExecCommand()`-backed exec handler -- plus the SDK's
 * onPermissionRequest callback. Nothing here computes settings; it only
 * wires handlers onto the shape the pure function already decided.
 */
export function buildAuditorSessionSettings(
  ...
  const pure = buildAuditorSessionSettingsPure(executionConfig, systemPrompt, tool);
  const [toolName, execToolName] = pure.allowedToolNames;
  return {
    model: pure.model,
    ...(pure.provider ? { provider: pure.provider as SdkProviderConfig } : {}),
    ...
    reasoningSummary: pure.reasoningSummary,
    ...
    tools: [
      {
        name: pure.submissionTool.name,
        description: pure.submissionTool.description,
        parameters: pure.submissionTool.parameters,
      ...
      {
        name: pure.execTool.name,
        description: pure.execTool.description,
        parameters: pure.execTool.parameters,
