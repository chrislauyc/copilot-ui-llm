...
import { ProviderRegistry } from './src/utils/providerRegistry';
import { getAuditorExecutionConfig, executeAuditSession } from './src/utils/auditorHelper';
...
import { getSession, saveSession, deleteSession, getAllSessions } from './src/db/sessionStore';
...
// Map to track SSE response locks to avoid heartbeat corruption
...
async function secureWrite(res: any, data: string, isRequestClosed: boolean = false) {
  ...
  const dataBrief = data.length > 100 ? `${data.slice(0, 100)}...` : data.trim();
        ...
        try {
          ...
          if (jsonStr) {
            ...
            if (eventObj && typeof eventObj === 'object') {
              // Extract data object or create one
              ...
              // Rewrite data string with sequence identifier included
  ...
  const nextLock = lock.then(() => {
    return new Promise<void>((resolve, reject) => {
      ...
      if (!canWrite) {
        ...
        // Handle close while polling for drain just in case
        res.once('close', () => {
           clearTimeout(timeoutId);
           resolve();
        });
  ...
  // Release reservation lock by resolving the catch branch so next items can execute if needed (or fail fast)
...
async function flushSseAndEnd(res: any): Promise<void> {
  ...
  // Brief wait to allow socket to fully flush out kernel buffers on slow connections
