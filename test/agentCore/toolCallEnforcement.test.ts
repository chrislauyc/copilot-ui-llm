...
import { runForcedToolTurn, sendAndWaitWithAbort, STALL_TIMEOUT_MS } from '../../src/agentCore/toolCallEnforcement';
...
describe('Upstream stall detection & retry (review-pr.ts stall-retry follow-up)', () => {
  ...
  describe('runForcedToolTurn stall-retry', () => {
    ...
    it('preserves the session via resumeSession on the first stall, avoiding history loss (issue #185/#192)', async () => {
      ...
      const runPromise = runForcedToolTurn(makeWrapper(mockClient), 'my_tool', initialPrompt, {
        maxRetries: 0,
        maxStallRetries: 2,
        getResult: () => ({ ok: true }),
        createFreshWrapper: () => makeWrapper(mockClient),
